...
mouse_melt$class <- factor(mouse_melt$class, levels = c('Protein-Coding Exons','CNEs'), labels = c(expression("Protein-Coding~Exons"),expression("CNEs")))
 ...
 ggplot(data = Mm, aes(x= dist, y = rat, col = lab))+
   geom_line()+
...
pdf('~/Work/MuridRodentTroughs/Plots/troughs.castaneus.pdf', width = 10, height = 7)
print(troughs)
dev.off()
...
pdf('~/Work/MuridRodentTroughs/Plots/trough_extras.castaneus.pdf', width = 10, height = 15)
print(troughs_extra)
dev.off()
