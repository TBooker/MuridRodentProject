...
Mmc     <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmc.liuDistance.csv')
Mmc$lab <- 'M. m. castaneus'
Mmc$rat <- Mmc$pi/JC(Mmc$div2)
Mmc$pi_ref <- Mmc$rat/ mean( Mmc[ (abs(Mmc$dist) > 1500) & abs(Mmc$dist) < 2500  , ]$rat ) 


Mmdg<-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmd_ger.liuDistance.csv')
...
Mmdf    <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmd_fra.liuDistance.csv')
...
Mmdi     <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmd_ira.liuDistance.csv')
...
Mmma     <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmm_afg.liuDistance.csv')
...
Mmmc     <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmm_cze.liuDistance.csv')
...
Mmmk     <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Mmm_kaz.liuDistance.csv')
...
Ms     <-read.csv('~/Work/MuridRodentTroughs/dataAnalysis/exons/Ms.liuDistance.csv')
...
pdf('Work/MuridRodentTroughs/Plots/exons_pi_genetic_liu.pdf', width =8.0, height = 5.5)
...
pdf('Work/MuridRodentTroughs/Plots/exons_piRef_genetic_liu.pdf', width =8.0, height = 5.5)
...
png('Work/MuridRodentTroughs/Plots/exons_piRef_genetic_liu.png', width =600, height = 400)
ggplot(data = Mm[Mm$dist>0,], aes(x= dist, y = pi_ref, col = lab))+
  geom_line(data = Mm[Mm$dist<0,], aes(x= abs(dist), y = pi_ref, col = lab), alpha = 0.75)+
  geom_line(alpha = 0.75)+
  scale_y_continuous(expression(pi/pi[Ref]))+
...
pdf('Work/MuridRodentTroughs/Plots/exons_divFam_genetic_liu.pdf', width =8.0, height = 5.5)
...
pdf('Work/MuridRodentTroughs/Plots/exons_divRat_genetic_liu.pdf', width =8.0, height = 5.5)
...
png('Work/TBooker.github.io/img/research/exons_divFam_genetic_liu.png', width =600, height = 400)
ggplot(data = Mm[Mm$dist>0,], aes(x= dist, y = pi_ref, col = lab))+
  geom_line(data = Mm[Mm$dist<0,], aes(x= abs(dist), y = pi_ref, col = lab), alpha = 0.75)+
  geom_line(alpha = 0.75)+
  scale_y_continuous(expression(pi/pi[Ref]))+
...
ggplot(data = Mm[Mm$dist>0,], aes(x= dist, y = sites, col = lab))+
  ...
  guides(colour = guide_legend(override.aes = list(size=3)))+
  theme_bw()+
#  scale_x_sqrt(expression('Genetic Distance from Exon ('*rho*' = '*italic('4N'[e]*'r')*')'), limits = c(1,3000))+
  TommyTheme
